...
class MLP(nn.Module):
	...
	def __init__(self, config):
		...
		self.c_proj.SCALE_INIT = 1 #flag to indicate we should scale init sd by 1/sqrt(n layer) to keep var at 1
...
class CausalSelfAttention(nn.Module):
	...
	def __init__(self, config):
		...
		# flash attention if available, if not, we use handcoded attention
	...
	def forward(self, x):
		...
		if self.flash:
			...
			#yields small speed up even w/o torch compile on mac m1
